    ...
    <div className="auth-container" style={{ background: '#050502' }}>
      <div className="auth-card" style={{ borderColor: 'var(--border)', background: 'var(--bg-subtle)' }}>
        <div className="auth-header">
          ...
          <h1 style={{ fontSize: '28px', fontWeight: '800', color: 'var(--text)' }}>Welcome Back</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', fontWeight: '500' }}>Continue your learning journey with AI.</p>
        </div>
        ...
        {resetEmailSent && (
          <div style={{ padding: '12px', background: 'rgba(34, 197, 94, 0.1)', color: '#16a34a', border: '1px solid #16a34a', borderRadius: '8px', fontSize: '13px', marginBottom: '20px', fontWeight: '600' }}>
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <label style={{ fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', color: 'var(--text-dim)', letterSpacing: '1px' }}>Password</label>
              <button type="button" onClick={handleResetPassword} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: '11px', fontWeight: '800', cursor: 'pointer' }}>Forgot?</button>
            </div>
            <input
              type="password"
              required
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', fontSize: '14px', color: 'var(--text-dim)', fontWeight: '500' }}>
          ...
          <Link href="/signup" style={{ color: 'var(--accent)', fontWeight: '800', textDecoration: 'none' }}>Sign up free</Link>
        </p>
      </div>
    </div>
  );
